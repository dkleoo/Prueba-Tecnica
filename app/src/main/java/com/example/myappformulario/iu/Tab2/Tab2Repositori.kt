...
class Tab2Repositori @Inject constructor(private val db:CustomersDatabase) {
    ...
    suspend fun deleteCustomer(document: Int) {
        return withContext(Dispatchers.IO){
            db.getCustomerDao().deleteCustomers(document.toInt())
        }
    }

    suspend fun updateCustomer(name: String, cid: Int) {
    ...
    suspend fun updateApellidoCustomer(apellido1: String, cid: Int) {
        withContext(Dispatchers.IO){
    ...
    suspend fun customerPhone(phone1: String, id: Int) {
        withContext(Dispatchers.IO){
            db.getCustomerPhoneDao().insertAll(CustomersPhoneEntity(
                0,
                id,
                phone1
            ))
